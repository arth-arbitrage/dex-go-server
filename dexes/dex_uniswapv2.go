...
func (dex *DexUniswapV2)GetSwapPools(ctx *DefaultApiService) (restapi.ImplResponse, error) {
	...
	factory, err := uniswapv2.NewIUniswapV2Factory(common.Address(common.HexToAddress(uniswapv2FactoryAddress)), client)
	if (err != nil) { fmt.Printf("Failed to get factory at %v",uniswapv2FactoryAddress)}
